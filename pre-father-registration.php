...
if(isset($_POST["photo_nic_mother"]) && $_POST["photo_nic_mother"] != "" && !empty($_FILES['file']))
{
    ...
    $first_name = $_POST["photo_first_name_mother"];
    $last_name = $_POST["photo_last_name_mother"];
...
<div class="page-container">
    ...
    <div class="page-content">
        ...
        <ul class="breadcrumb">
            ...
            <li class="active">New Registration - Pre Birth</li>
        </ul>

        <div class="page-title">
            <h2 class="text-uppercase">New Registration - Pre Birth</h2>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-default">
                        ...
                        <?php require_once('pages/pre-birth/father-details.php'); ?>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('.father_data').on('submit', function(e){
        e.preventDefault();
        $.ajax({
            url : 'http://54.166.227.166:3000/api/save/father',
            type : 'post',
            dataType: 'json',
            processData: false,
            data: JSON.stringify(getFormData($('.father_data'))),
            ...
            success : function(r){
                if (r.status) {
                    $('#photo_nic_father').val(r.data.nic_no);
                    $('#photo_nic_mother').val(r.data.mother_nic_no);
                    $('#photo_first_name_father').val(r.data.first_name);
                    $('#photo_last_name_father').val(r.data.last_name);

                    swal({
                        title: "Registration Complete",
                        text: "All Required Details Saved Successfully",
                        icon: "success",
                        buttons: [null,'OK'],
                    }).then(function(isConfirm) {
                        $('#photo_upload_father').submit();
                    });
                }
            }
        });
